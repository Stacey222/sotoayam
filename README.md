...
Environment wajib: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `TELEGRAM_BOT_TOKEN`, dan `INTERNAL_API_KEY`. Isi `TELEGRAM_BOT_USERNAME` (tanpa `@`) untuk mengaktifkan tautan pairing customer dari dashboard. `ADMIN_API_KEY` hanya diperlukan bila fallback kompatibilitas diaktifkan, dan minimal 32 karakter. Untuk kompatibilitas instalasi lama, backend juga menerima alias `SUPABASE_SERVICE_KEY`, tetapi nama canonical yang dianjurkan adalah `SUPABASE_SERVICE_ROLE_KEY`.
