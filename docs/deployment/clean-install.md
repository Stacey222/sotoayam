...
| `NOTIFICATION_PREFERENCE_RESOLVER_MODE` | Optional | Use `NORMALIZED` for the approved authoritative read path. `COMPARE` logs sanitized parity counts while delivering from legacy recipients; `LEGACY` is the rollback path. |
