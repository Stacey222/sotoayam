export function setupSettingsView({ api, showGlobal }) {
  ...
  function render(data) {
    ...
    $("#settings-source").textContent = data.source === "RUNTIME" ? "Runtime" : "Default deployment";
    ...
    $("#settings-updated").textContent = data.updated_at ? `Diperbarui ${new Intl.DateTimeFormat("id-ID", { dateStyle: "medium", timeStyle: "short" }).format(new Date(data.updated_at))}` : "Belum pernah diubah saat runtime";
    $("#settings-business-current").textContent = data.business_actor?.display_name || "Belum ditetapkan";
  ...
  async function candidates() {
    ...
    const select = $("#settings-business-actor"); select.replaceChildren(new Option("Pilih pengguna yang memenuhi syarat", ""));
  ...
  async function load() {
    ...
    } catch (error) { notice(error.status === 403 ? "Sesi ini tidak memiliki akses ke pengaturan runtime." : "Pengaturan runtime tidak dapat dimuat."); }
  }
  $("#settings-form").addEventListener("submit", async (event) => {
    event.preventDefault(); if (!state.settings || !confirm("Terapkan pengaturan runtime ini sekarang?")) return;
    ...
    try { const payload = await api("/api/admin/settings/runtime", { method: "PATCH", body: JSON.stringify({
      ...
      form.elements.reason.value = ""; render(payload.data); notice("Pengaturan runtime berhasil diterapkan.", false);
    } catch (error) { notice(error.status === 409 ? "Pengaturan berubah atau tidak berbeda. Muat ulang sebelum mencoba lagi." : error.message); }
  });
  $("#business-actor-form").addEventListener("submit", async (event) => {
    event.preventDefault(); if (!state.settings || !confirm("Ganti penanggung jawab bisnis?")) return;
    ...
    try { const payload = await api("/api/admin/settings/business-actor", { method: "PATCH", body: JSON.stringify({
      ...
      form.elements.reason.value = ""; render(payload.data); notice("Penanggung jawab bisnis berhasil diperbarui.", false);
    } catch (error) { notice(error.status === 409 ? "Pengguna tidak memenuhi syarat atau versi pengaturan sudah berubah." : error.message); }
  });
  return { load };
}
