...
export class SupabaseTaskUsersRepository implements TaskUsersRepository {
  constructor(private readonly client: SupabaseClient) {}

  async findById(id: number): Promise<TaskUser | null> {
    const { data, error } = await this.client
      .from("users").select("id,active,division_id,role_id,roles(code)").eq("id", id).maybeSingle();
    if (error) throw governanceDatabaseError("Unable to load normalized task user", error);
    if (!data) return null;
    const row = data as unknown as TaskUserRow;
    return { id: row.id, active: row.active, divisionId: row.division_id, roleId: row.role_id, roleCode: row.roles?.code ?? null };
  }
