...
export async function criticalAlertsRoutes(app: FastifyInstance, options: { service: CriticalAlertService; actorResolver: OwnerActorResolver; adminApiKey?: string }): Promise<void> {
  ...
  app.get("/automation-status", async (request) => ({ success: true, data: await options.service.automationStatus(await options.actorResolver.resolveOwnerActor(request.headers.authorization)) }));
  app.get("/:id", async (request) => ({ success: true, data: await options.service.get(await options.actorResolver.resolveOwnerActor(request.headers.authorization), positiveId((request.params as { id: string }).id)) }));
  app.post("/:id/acknowledge", async (request) => ({ success: true, data: await options.service.acknowledge(await options.actorResolver.resolveOwnerActor(request.headers.authorization), positiveId((request.params as { id: string }).id)) }));
}
