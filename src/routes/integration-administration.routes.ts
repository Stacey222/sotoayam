...
export async function integrationAdministrationRoutes(app: FastifyInstance, options: IntegrationAdministrationRoutesOptions) {
  ...
  app.post("/", async (request, reply) => {
    ...
    const data = await options.service.create(await actor(request), { code: value.code, name: value.name, source: value.source as "AUTOMATION" | "ERP", requestingDivisionId: value.requesting_division_id });
  ...
  app.patch<{ Params: { id: string } }>("/:id/active", async (request) => {
    ...
    return { success: true, data: await options.service.setActive(await actor(request), parsePositiveId(request.params.id), value.active) };
  });
  app.get<{ Params: { id: string } }>("/:id/capabilities", async (request) => ({
    success: true, data: await options.service.listCapabilities(await actor(request), parsePositiveId(request.params.id)),
  }));
  app.put<{ Params: { id: string; capability: string } }>("/:id/capabilities/:capability", async (request) => ({
    success: true, data: await options.service.grantCapability(await actor(request), parsePositiveId(request.params.id), request.params.capability),
  }));
  app.delete<{ Params: { id: string; capability: string } }>("/:id/capabilities/:capability", async (request) => ({
    success: true, data: await options.service.revokeCapability(await actor(request), parsePositiveId(request.params.id), request.params.capability),
  }));
}
