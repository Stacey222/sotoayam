...
export const taxonomyRoutes = defineAdminRoutes<TaxonomyRoutesOptions>(async (app, options) => {
  const actor = async () => {
    const resolved = await options.actorResolver.resolveTrustedActor();
  ...
  app.post("/divisions", async (request, reply) => {
    ...
    return reply.status(201).send({ success: true, data: await options.service.createDivision({ code: value.code, name: value.name }, await actor()) });
  });
  app.patch<{ Params: { id: string } }>("/divisions/:id", async (request) => {
    ...
    return { success: true, data: await options.service.updateDivision(parsePositiveId(request.params.id), value as { name?: string; active?: boolean }, await actor()) };
  });
  app.delete<{ Params: { id: string } }>("/divisions/:id", async (request) => ({
    success: true, data: await options.service.deleteDivision(parsePositiveId(request.params.id), await actor()),
  }));

  app.get("/roles", async () => { await actor(); return { success: true, data: await options.service.listRoles() }; });
  app.patch<{ Params: { id: string } }>("/roles/:id", async (request) => {
    ...
    return { success: true, data: await options.service.renameRole(parsePositiveId(request.params.id), value.name, await actor()) };
  });

  app.get("/task-categories", async () => { await actor(); return { success: true, data: await options.categories.list() }; });
  app.post("/task-categories", async (request, reply) => {
    const value = body(request.body); fields(value, ["code", "name"]);
    if (typeof value.code !== "string" || typeof value.name !== "string") throw new AppError(400, "VALIDATION_ERROR", "code and name are required");
    return reply.status(201).send({ success: true, data: await options.categories.create({ code: value.code, name: value.name }, (await actor()).id) });
  });
  app.patch<{ Params: { id: string } }>("/task-categories/:id", async (request) => {
    ...
    if (Object.keys(value).length === 0 || (value.name !== undefined && typeof value.name !== "string")
      || (value.active !== undefined && typeof value.active !== "boolean")) throw new AppError(400, "VALIDATION_ERROR", "Provide a valid name and/or active value");
    return { success: true, data: await options.categories.update(parsePositiveId(request.params.id), value as { name?: string; active?: boolean }, (await actor()).id) };
  });
  app.delete<{ Params: { id: string } }>("/task-categories/:id", async (request) => ({
    success: true, data: await options.categories.delete(parsePositiveId(request.params.id), (await actor()).id),
  }));
});
