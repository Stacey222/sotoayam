...
import { commonMessages, formatCompletePreview, formatInvalidBlockReason, formatInvalidComment,
  formatInvalidDescription, formatTaskButton, formatTaskDetail, formatTaskInputPrompt, formatTaskList,
  formatTaskMenu, formatTaskReview, formatTaskSummary, taskErrorMessage, telegramButtons,
  telegramTaskMessages } from "../messages/catalog.js";
...
const FILTERS: Readonly<Record<FilterCode, { label: string; statuses: readonly TaskStatus[] }>> = {
  o: { label: telegramTaskMessages.filters.open, statuses: ["OPEN"] },
  i: { label: telegramTaskMessages.filters.inProgress, statuses: ["IN_PROGRESS"] },
  b: { label: telegramTaskMessages.filters.blocked, statuses: ["BLOCKED"] },
  c: { label: telegramTaskMessages.filters.completed, statuses: ["COMPLETED"] },
  a: { label: telegramTaskMessages.filters.allActive, statuses: ACTIVE_STATUSES },
};
...
export class TelegramTaskConsoleService implements TelegramTaskConsole {
  ...
  async handleCallback(externalTelegramId: number, data: string): Promise<TelegramConsoleResponse> {
    ...
    try {
      ...
      if (data === "tc:cx") { this.states.delete(externalTelegramId); return this.mainMenu(actor, telegramTaskMessages.creationCancelled); }
  ...
  async handleText(externalTelegramId: number, rawText: string): Promise<TelegramConsoleResponse | null> {
    ...
    try {
      ...
      return this.safeError(error, [[button(telegramButtons.cancel, "tc:ix")]]);
    }
  }
  ...
  private mainMenu(actor: TaskActor, notice?: string): TelegramConsoleResponse {
    const keyboard: TelegramInlineButton[][] = [[button(telegramTaskMessages.buttons.myTasks, "tc:l")]];
    if (actor.permissions.has("task.create")) keyboard.push([button(telegramTaskMessages.buttons.createTask, "tc:c")]);
    return { text: formatTaskMenu(notice), inlineKeyboard: keyboard };
  }

  private filterMenu(): TelegramConsoleResponse {
    return { text: telegramTaskMessages.filterPrompt, inlineKeyboard: [
      [button(telegramTaskMessages.filters.open, "tc:p:o:0"), button(telegramTaskMessages.filters.inProgress, "tc:p:i:0")],
      [button(telegramTaskMessages.filters.blocked, "tc:p:b:0"), button(telegramTaskMessages.filters.completed, "tc:p:c:0")],
      [button(telegramTaskMessages.filters.allActive, "tc:p:a:0")],
      [button(telegramButtons.back, "tc:m")],
    ] };
  }
  ...
  private async taskList(actor: TaskActor, filter: FilterCode, requestedPage: number): Promise<TelegramConsoleResponse> {
    ...
    const text = formatTaskList(definition.label, visible.map(formatTaskSummary), page, pages);
    const keyboard = visible.map((task) => [button(formatTaskButton(task.id, task.title), `tc:d:${task.id}:${filter}:${page}`)]);
    const navigation: TelegramInlineButton[] = [];
    if (page > 0) navigation.push(button(telegramButtons.previous, `tc:p:${filter}:${page - 1}`));
    if (page + 1 < pages) navigation.push(button(telegramButtons.next, `tc:p:${filter}:${page + 1}`));
    if (navigation.length) keyboard.push(navigation);
    keyboard.push([button(telegramButtons.back, "tc:l")]);
  ...
  private async taskDetail(actor: TaskActor, taskId: number, context: NavigationContext, notice?: string): Promise<TelegramConsoleResponse> {
    ...
    const requesting = divisionEntries.find((item) => item.id === task.requesting_division_id)?.code ?? commonMessages.unavailable;
    const owner = divisionEntries.find((item) => item.id === task.owner_division_id)?.code ?? commonMessages.unavailable;
    ...
    if (task.status === "OPEN" && canUpdate) actions.push([button(telegramTaskMessages.buttons.startTask, this.action("t:s", task, context))]);
    if (task.status === "IN_PROGRESS" && canUpdate) actions.push([button(telegramTaskMessages.buttons.blockTask, this.action("q:b", task, context))]);
    if (task.status === "BLOCKED" && canUpdate) actions.push([button(telegramTaskMessages.buttons.resumeTask, this.action("t:r", task, context))]);
    if (["OPEN", "IN_PROGRESS", "BLOCKED"].includes(task.status) && canComplete) {
      actions.push([button(telegramTaskMessages.buttons.completeTask, this.action("v:c", task, context))]);
    }
    if (canComment) actions.push([button(telegramTaskMessages.buttons.addComment, this.action("q:c", task, context))]);
    actions.push([button(telegramButtons.back, `tc:p:${context.filter}:${context.page}`)]);
    return { text: formatTaskDetail({ task, requesting, owner,
      assignee: assignee?.displayName?.trim() || telegramTaskMessages.buttons.unassigned, notice }), inlineKeyboard: actions };
  }
  ...
  private async beginTaskInput(externalTelegramId: number, actor: TaskActor, kind: "comment" | "block", taskId: number, context: NavigationContext): Promise<TelegramConsoleResponse> {
    ...
    return {
      text: formatTaskInputPrompt(kind, taskId, MAX_COMMENT),
      inlineKeyboard: [[button(telegramButtons.cancel, "tc:ix")]],
    };
  }
  ...
  private async submitComment(externalTelegramId: number, actor: TaskActor, state: TaskInputState, text: string): Promise<TelegramConsoleResponse> {
    if (!text || text.length > MAX_COMMENT) return { text: formatInvalidComment(MAX_COMMENT), inlineKeyboard: [[button(telegramButtons.cancel, "tc:ix")]] };
    const key = `${actor.id}:comment:${state.taskId}`;
    if (this.inFlight.has(key)) return { text: telegramTaskMessages.commentProcessing };
    this.inFlight.add(key);
    try {
      await this.service.addActivity(actor, state.taskId, { activityType: "COMMENT", note: text, visibility: "SHARED" });
      this.states.delete(externalTelegramId);
      return this.taskDetail(actor, state.taskId, state, telegramTaskMessages.commentAdded);
  ...
  private async submitBlock(externalTelegramId: number, actor: TaskActor, state: TaskInputState, text: string): Promise<TelegramConsoleResponse> {
    if (!text || text.length > MAX_COMMENT) return { text: formatInvalidBlockReason(MAX_COMMENT), inlineKeyboard: [[button(telegramButtons.cancel, "tc:ix")]] };
    const key = `${actor.id}:block:${state.taskId}`;
    if (this.inFlight.has(key)) return { text: telegramTaskMessages.statusProcessing };
    this.inFlight.add(key);
    try {
      await this.service.transition(actor, state.taskId, { status: "BLOCKED", note: text });
      this.states.delete(externalTelegramId);
      return this.taskDetail(actor, state.taskId, state, telegramTaskMessages.taskBlocked);
    } finally { this.inFlight.delete(key); }
  }
  ...
  private async transition(actor: TaskActor, action: string, taskId: number, context: NavigationContext): Promise<TelegramConsoleResponse> {
    ...
    try {
      ...
      const notice = status === "COMPLETED" ? telegramTaskMessages.taskCompleted : action === "r" ? telegramTaskMessages.taskResumed : telegramTaskMessages.taskStarted;
  ...
  private async completePreview(actor: TaskActor, taskId: number, context: NavigationContext): Promise<TelegramConsoleResponse> {
    const task = await this.service.get(actor, taskId);
    return { text: formatCompletePreview(task.id, task.title), inlineKeyboard: [
      [button(telegramTaskMessages.buttons.confirmComplete, this.action("t:c", task, context))],
      [button(telegramButtons.back, this.action("d", task, context))],
    ] };
  }
  ...
  private beginCreate(externalTelegramId: number, actor: TaskActor): TelegramConsoleResponse {
    ...
    return { text: telegramTaskMessages.createTitlePrompt, inlineKeyboard: [[button(telegramButtons.cancel, "tc:cx")]] };
  }
  ...
  private async handleCreateText(externalTelegramId: number, actor: TaskActor, state: CreateState, text: string): Promise<TelegramConsoleResponse> {
    if (state.step === "title") {
      if (!text || text.length > 200) return { text: telegramTaskMessages.invalidTitle, inlineKeyboard: [[button(telegramButtons.cancel, "tc:cx")]] };
      state.draft.title = text; state.step = "description"; this.touch(state);
      return { text: telegramTaskMessages.descriptionPrompt, inlineKeyboard: [[button(telegramButtons.skip, "tc:cs:d"), button(telegramButtons.cancel, "tc:cx")]] };
    }
    if (state.step === "description") {
      if (text.length > MAX_DESCRIPTION) return { text: formatInvalidDescription(MAX_DESCRIPTION), inlineKeyboard: [[button(telegramButtons.skip, "tc:cs:d"), button(telegramButtons.cancel, "tc:cx")]] };
    ...
    if (state.step === "deadline") {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(text) || !this.validDate(text)) {
        return { text: telegramTaskMessages.invalidDeadline, inlineKeyboard: [[button(telegramButtons.skip, "tc:cd:s"), button(telegramButtons.cancel, "tc:cx")]] };
      }
  ...
  private priorityPrompt(state: CreateState): TelegramConsoleResponse {
    ...
    return { text: telegramTaskMessages.priorityPrompt, inlineKeyboard: [
      [button("LOW", "tc:cp:L"), button("NORMAL", "tc:cp:N")],
      [button("HIGH", "tc:cp:H"), button("URGENT", "tc:cp:U")],
      [button(telegramButtons.cancel, "tc:cx")],
    ] };
  }
  ...
  private async choosePriority(externalTelegramId: number, actor: TaskActor, priority: TaskPriority): Promise<TelegramConsoleResponse> {
    ...
    return { text: telegramTaskMessages.ownerPrompt, inlineKeyboard: [
  ...
  private async chooseOwner(externalTelegramId: number, actor: TaskActor, ownerDivisionId: number): Promise<TelegramConsoleResponse> {
    ...
    return { text: telegramTaskMessages.assigneePrompt, inlineKeyboard: [
      [button(telegramTaskMessages.buttons.unassigned, "tc:ca:0")],
  ...
  private async chooseAssignee(externalTelegramId: number, actor: TaskActor, assigneeId: number): Promise<TelegramConsoleResponse> {
    ...
    return { text: telegramTaskMessages.deadlinePrompt, inlineKeyboard: [[button(telegramButtons.skip, "tc:cd:s"), button(telegramButtons.cancel, "tc:cx")]] };
  }
  ...
  private async review(actor: TaskActor, state: CreateState): Promise<TelegramConsoleResponse> {
    ...
    const requesting = divisions.find((item) => item.id === actor.divisionId)?.code ?? commonMessages.unavailable;
    const owner = divisions.find((item) => item.id === state.draft.ownerDivisionId)?.code ?? commonMessages.unavailable;
    return { text: formatTaskReview({ title: state.draft.title, description: state.draft.description,
      priority: state.draft.priority, requesting, owner,
      assignee: assignee?.displayName?.trim() || telegramTaskMessages.buttons.unassigned, deadline: state.draft.deadline }),
      inlineKeyboard: [[button(telegramTaskMessages.buttons.confirmCreate, "tc:cc")], [button(telegramButtons.cancel, "tc:cx")]] };
  }
  ...
  private async confirmCreate(externalTelegramId: number, actor: TaskActor): Promise<TelegramConsoleResponse> {
    ...
    try {
      ...
      this.states.delete(externalTelegramId);
      return this.taskDetail(actor, task.id, { filter: "o", page: 0 }, telegramTaskMessages.taskCreated);
    } catch (error) {
  ...
  private async cancelInput(externalTelegramId: number, actor: TaskActor): Promise<TelegramConsoleResponse> {
    const state = this.currentState(externalTelegramId, actor.id);
    this.states.delete(externalTelegramId);
    if (state?.kind === "comment" || state?.kind === "block") return this.taskDetail(actor, state.taskId, state, telegramTaskMessages.inputCancelled);
    return this.mainMenu(actor, telegramTaskMessages.inputCancelled);
  }
  ...
  private safeError(error: unknown, keyboard?: TelegramInlineButton[][]): TelegramConsoleResponse {
    return { text: error instanceof AppError ? taskErrorMessage(error.code) : commonMessages.requestFailed, inlineKeyboard: keyboard };
  }
}
