...
describe("OWNER alert authorization and lifecycle", () => {
  ...
  it("preserves shared-key reads but requires a real OWNER session for acknowledgement", async () => { const alerts = new Alerts(); await seed(alerts); const app = Fastify(); await app.register(criticalAlertsRoutes, { service: alertService(new Signals(), alerts), actorResolver: { resolveOwnerActor: async () => owner() }, adminApiKey: "key" }); expect((await app.inject({ method: "GET", url: "/", headers: { "x-admin-api-key": "key" } })).statusCode).toBe(200); const mutation = await app.inject({ method: "POST", url: "/1/acknowledge", headers: { "x-admin-api-key": "key" } }); expect(mutation.statusCode).toBe(401); expect(mutation.json().message).toContain("session"); await app.close(); });
});
