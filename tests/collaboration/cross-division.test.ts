...
function managementHarness(options: { authority?: boolean; normalized?: Partial<ManagedUser> } = {}) {
  ...
  const authenticatedActor: TaskActor = { id: 9, displayName: "IT Admin", active: managed.value.active,
    divisionId: managed.value.division?.id ?? null, divisionCode: managed.value.division?.code ?? null,
    roleId: managed.value.role?.id ?? null, roleCode: managed.value.role?.code ?? null, permissions: new Set() };
  return { service: new CollaborationRuleManagementService(rules, divisionsRepo, authorities, audit), rules, audit, managed, actor: authenticatedActor };
}
...
describe("Slice 4 Cross-Divisi collaboration", () => {
  ...
  it.each([["21. STAFF", "STAFF"], ["22. business ADMIN", "ADMIN"], ["23. OWNER", "OWNER"]])("%s cannot manage rules without SYSTEM_ADMIN", async (_label, code) => { const h = managementHarness({ authority: false, normalized: { role: roles.find((r) => r.code === code) } }); await expect(h.service.list(h.actor)).rejects.toMatchObject({ code: "COLLABORATION_GOVERNANCE_FORBIDDEN" }); });
  it("24. active IT SYSTEM_ADMIN can manage rules", async () => { const h = managementHarness(); expect((await h.service.create(h.actor, { sourceDivisionId: 10, targetDivisionId: 20, taskScope: "ALL", allowed: true, requiresApproval: false })).active).toBe(true); });
  it("25. rule creation is audited", async () => { const h = managementHarness(); await h.service.create(h.actor, { sourceDivisionId: 10, targetDivisionId: 20, taskScope: "ALL", allowed: true, requiresApproval: false }); expect(h.audit.rows[0]?.actor_user_id).toBe(h.actor.id); });
  it("26. rule update is audited", async () => { const h = managementHarness(); const rule = h.rules.seed(); await h.service.update(h.actor, rule.id, { requiresApproval: true }); expect(h.audit.rows[0]?.action).toBe("COLLABORATION_RULE_UPDATED"); });
  it("27. rule disable is audited", async () => { const h = managementHarness(); const rule = h.rules.seed(); await h.service.deactivate(h.actor, rule.id); expect(h.audit.rows[0]?.action).toBe("COLLABORATION_RULE_DISABLED"); });
  it("28. duplicate active rule rejected", async () => { const h = managementHarness(); h.rules.seed(); await expect(h.service.create(h.actor, { sourceDivisionId: 10, targetDivisionId: 20, taskScope: "ALL", allowed: true, requiresApproval: false })).rejects.toMatchObject({ code: "COLLABORATION_DUPLICATE_RULE" }); });
  it("29. source=target rejected", async () => { const h = managementHarness(); await expect(h.service.create(h.actor, { sourceDivisionId: 10, targetDivisionId: 10, taskScope: "ALL", allowed: true, requiresApproval: false })).rejects.toMatchObject({ code: "COLLABORATION_INVALID_TARGET_DIVISION" }); });
