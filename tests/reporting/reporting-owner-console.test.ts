...
describe("protected reporting API and schema contract", () => {
  ...
  it("forwards the verified user session to OWNER resolution", async () => { const app = Fastify(); const resolveOwnerActor = vi.fn().mockResolvedValue(owner()); await app.register(reportsRoutes, { service: service([task()]), actorResolver: { resolveOwnerActor }, adminApiKey: "key" }); const response = await app.inject({ method: "GET", url: "/content-creator/affiliate-task-status?window=TODAY", headers: { "x-admin-api-key": "key", authorization: "Bearer owner-session" } }); expect(response.statusCode).toBe(200); expect(response.json().data.total).toBe(1); expect(resolveOwnerActor).toHaveBeenCalledWith("Bearer owner-session"); await app.close(); });
