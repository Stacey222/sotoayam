...
describe("P2-09 Admin & User Management acceptance", () => {
  ...
  it("U-19 has exactly 20 migrations and preserves the nineteen historical bytes", async () => {
    const names = (await readdir(path.resolve("supabase/migrations"))).filter((name) => name.endsWith(".sql")).sort();
    expect(names).toHaveLength(20); expect(names.at(-1)).toBe("202609140001_create_admin_user_management.sql");
